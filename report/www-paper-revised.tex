...
Raghavan et al.~\cite{raghavan2007} address distributed rate limiting in cloud environments, proposing algorithms that maintain global rate limits across geographically distributed enforcement points. Their work focuses on \emph{incoming} request rate limiting (protecting the server), whereas the constraint we faced was on \emph{outgoing} requests, that is operating inside an external provider's quota, which is a distinct problem and, as Section~\ref{subsec:roundrobin} argues, largely a commercial rather than an algorithmic one.

Commercial transactional email platforms document per-account quotas and offer higher allowances through paid plans~\cite{brevo-limits}. We are not aware of formal treatment of client-side rotation across multiple accounts to enlarge an aggregate free allowance, and we now think the absence of such treatment reflects the nature of the practice rather than a gap in the literature. Section~\ref{subsec:roundrobin} describes what we built, explains why we withdraw it, and sets out the legitimate ways to obtain more capacity.
...
\textbf{Data Layer.} The NoSQL document store provides real-time subscriptions. The authentication service provides custom token infrastructure for client-side database access. External services include the transactional email API and the edge CDN for static asset delivery.
...
\textbf{Intended load distribution.} Within the lifetime of a single process the counter causes successive requests to use different credentials, so that over $m$ requests served by one instance each credential receives about $m/n$. This is the design intent rather than an observed property, and the qualifier matters: as noted below, instance recycling resets the counter, and we did not record the distribution actually achieved.
